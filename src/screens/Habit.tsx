...
import { generateProgressPercentage } from "../shared/utils/generate-progress-percentage";
...
export function HabitScreen() {
  ...
  async function fetchHabits() {
    try {
      ...
      const response = await api.get(`/${JSON.parse(user!).id}/day`, {
        params: {
          date,
        },
      });
  ...
  async function handleToggleHabit(habidId: string) {
    try {
      const user = await AsyncStorage.getItem("@user");
      await api.patch(`/${JSON.parse(user!).id}/habits/${habidId}/toggle`);
